...
**Property & Casualty Insurance Actuarial Data Warehouse: Chain-ladder loss reserving triangles, Combined Ratio underwriting profit, CAT loss exceedance PML, and claims velocity.**

[![CI Analytics Pipeline](https://github.com/abdussatarkhan/InsurEdge-Property-Casualty-Actuarial-Analytics/actions/workflows/ci.yml/badge.svg)](https://github.com/abdussatarkhan/InsurEdge-Property-Casualty-Actuarial-Analytics/actions)
...
[![Maintenance](https://img.shields.io/badge/Maintained%3F-yes-brightgreen.svg?style=flat-square)](https://github.com/abdussatarkhan/InsurEdge-Property-Casualty-Actuarial-Analytics)
...
**InsurEdge-Property-Casualty-Actuarial-Analytics** is an enterprise-grade analytics data warehouse and business intelligence platform built for **P&C Insurance & Actuarial Science**.
...
### 1️⃣ Executive KPI Command Center (Actuarial Steel & Ocean Blue (Clean Light))
...
```
                    ...
                    └────────────┬────────────┘
                                 ...
                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_policyholders │──►│     fact_policies_written │◄──┤     dim_lines_of_business │
└──────────────────────┘   └─────────────┬───────────┘   └─────────────────────────┘
                                         │
       ┌─────────────────────────────────┼─────────────────────────────────┐
       ▼                                 ▼                                 ▼
┌──────────────────────┐   ┌─────────────────────────┐   ┌─────────────────────────┐
│     dim_coverage_types │   │     dim_geographic_zones │   │     dim_date         │
└──────────────────────┘   └─────────────────────────┘   └─────────────────────────┘
```
...
FROM insuredge_dw.fact_policies_written f
JOIN insuredge_dw.dim_date dt ON f.date_key = dt.date_key
...
```bash
git clone https://github.com/abdussatarkhan/InsurEdge-Property-Casualty-Actuarial-Analytics.git
cd InsurEdge-Property-Casualty-Actuarial-Analytics
